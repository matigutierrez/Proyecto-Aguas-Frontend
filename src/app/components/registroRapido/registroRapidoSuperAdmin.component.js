(function () {
  ...
  .component('registroRapidoSuperAdmin', {
    templateUrl: 'app/components/registroRapido/registroRapidoSuperAdmin.html',
    controller: registroRapidoSuperAdminCtr,
    controllerAs: 'vm'
  });

  registroRapidoSuperAdminCtr.$inject = ['UsuarioLogService', 'SubsidioService', 'EstadoService', 'ComunaService', 'EstadoMedidorService', 'ViviendaService', 'MedidorService', 'ClienteService', 'ComiteService', '$state', '$mdDialog', '$rootScope'];

  var dataComit = {};

  function registroRapidoSuperAdminCtr(UsuarioLogService, SubsidioService, EstadoService, ComunaService, EstadoMedidorService, ViviendaService, MedidorService, ClienteService, ComiteService, $state, $mdDialog, $rootScope) {
    var vm = this;
          ...
          if(clienteCreado == true && viviendaCreada == true && medidorCreado == true){
            $mdDialog.show(
              $mdDialog.alert()
                  ...
                  .textContent('Nombre: ' + cliente.nombre + ' ' + cliente.apellido_pater + ',\n Dirección: ' + vivienda.direccion + ',\n Medidor: ' + medidor.num_medidor)
                  .ok('Ok!')
            );
